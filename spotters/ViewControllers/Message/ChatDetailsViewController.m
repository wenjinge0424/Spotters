...
- (void)viewDidLoad {
    ...
    [self.inputToolbar setBackgroundColor:[UIColor colorWithRed:70/255.f green:24/255.f blue:125/255.f alpha:1]];
    self.inputToolbar.contentView.textView.placeHolder = @"Enter Message";
    [self.inputToolbar.contentView setBackgroundColor:[UIColor colorWithRed:70/255.f green:24/255.f blue:125/255.f alpha:1]];
...
- (void)loadMessages {
    if (!isLoading) {
        ...
        PFUser * roomSender = self.room[PARSE_ROOM_SENDER];
        if ([roomSender.objectId isEqualToString:me.objectId]){
            [query whereKey:PARSE_ROOM_SENDER_REMOVE notEqualTo:[NSNumber numberWithBool:YES]];
        }else{
...
- (void)sendMessage:(NSString *)text video:(PFFile *)video photo:(PFFile *)photo {
    ...
    [object saveInBackgroundWithBlock:^(BOOL succeeded, NSError * _Nullable error) {
        if (succeeded) {
            ...
            PFUser * room_sender = self.room[PARSE_ROOM_SENDER];
            self.room[PARSE_ROOM_SENDER_REMOVE] = [NSNumber numberWithBool:NO];
            self.room[PARSE_ROOM_RECEIVER_REMOVE] = [NSNumber numberWithBool:NO];
...
- (void)sendPushNotification:(NSString *)msg {
    ...
    if (isEnableChatNotify) {
        NSString *name = [NSString stringWithFormat:@"%@ %@", me[PARSE_USER_FIRSTNAME], me[PARSE_USER_LASTSTNAME]];
    ...
    if (!msg.isMediaMessage) {
        ...
        if ([msg.senderId isEqualToString:self.senderId]) {
            ...
            if (!me[PARSE_USER_AVATAR])
                cell.avatarImageView.image = [JSQMessagesAvatarImageFactory circularAvatarImage:[UIImage imageNamed:@"ico_userProfile"] withDiameter:48];
            else{
...
- (void) textFieldDidEndEditing:(UITextField *)textField
{
    ...
    [query whereKey:PARSE_USER_EMAIL notEqualTo:textField.text];
